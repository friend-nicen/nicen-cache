 ...
 * 加载主题设置页面
 * */
function nicen_cache_setting_load( $options ) {
                ...
                >
                    ...
                    <div class="card-container">
                        <a-tabs type="card" v-model="activeKey" @change="change">
							<?php
							//输出输入域
							nicen_cache_do_settings_sections_user( $plugin_page );
							?>
                        </a-tabs>
                    </div>
                </a-form>
 ...
 * */
function nicen_cache_do_settings_fields_user( $page, $section, $callback = false ) {
 ...
 * 主题设置片段页面输出
 * */
function nicen_cache_do_settings_sections_user( $page ) {
	...
	foreach ( (array) $wp_settings_sections[ $page ] as $key => $section ) {
		...
		nicen_cache_do_settings_fields_user( $page, $section['id'], $section['callback'] ?? false );
 ...
 * 数字输入框
 * */
function nicen_cache_form_number( $args ) {
	?>
    <a-input-number
            name="<?php echo esc_html( $args['label_for'] ); ?>"
            style="width: 100%;"
            placeholder="请输入<?php echo esc_html( $args['title'] ); ?>"
            v-model="data.<?php echo esc_html( $args['label_for'] ); ?>"></a-input-number>
	<?php
}
 ...
 * 基础输入框
 * */
function nicen_cache_form_input( $args ) {
	?>
    <a-input
            name="<?php echo esc_html( $args['label_for'] ); ?>"
            placeholder="请输入<?php echo esc_html( $args['title'] ); ?>"
 ...
 * 基础密码输入框
 * */
function nicen_cache_form_password( $args ) {
	?>
    <a-input-password
            name="<?php echo esc_html( $args['label_for'] ); ?>"
            placeholder="请输入<?php echo esc_html( $args['title'] ); ?>"
 ...
 * 基础开关
 * */
function nicen_cache_form_switch( $args ) {
	?>
    ...
    <a-switch
            ...
            @change="(checked,events)=>{hasChange(checked,events,'<?php echo esc_html( $args['label_for'] ); ?>')}"></a-switch>
	<?php
}
 ...
 * 基础开关
 * */
function nicen_cache_form_textarea( $args ) {
	?>
    <a-textarea
            name="<?php echo esc_html( $args['label_for'] ); ?>"
            v-model="data.<?php echo esc_html( $args['label_for'] ); ?>"
 ...
 * 基础开关
 * */
function nicen_cache_form_color( $args ) {
	?>
 ...
 * 文字说明
 * */
function nicen_cache_form_text( $args ) {
	?>
 ...
 * 单选
 * */
function nicen_cache_form_select( $args ) {
	?>
    <input name="<?php echo esc_html( $args['label_for'] ); ?>"
           v-model="data.<?php echo esc_html( $args['label_for'] ); ?>"
           hidden/>
    <a-select
            ...
            show-arrow
            v-model="data.<?php echo esc_html( $args['label_for'] ); ?>"
            placeholder="请选择<?php echo esc_html( $args['title'] ); ?>"></a-select>
	<?php
}
 ...
 * 单选
 * */
function nicen_cache_form_multi( $args ) {
 ...
 * 媒体选择器
 * */
function nicen_cache_form_media( $args ) {
	?>
    <div style="width: 100%;display: flex;">
        ...
        <a-button @click="showMedia('<?php echo $args['label_for']; ?>')"
                  style="manicen_cache_form_textrgin-left: 15px;"
                  type="plain">
            选择
        </a-button>
    </div>
